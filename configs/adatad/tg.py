...
def _compute_class_weights():
    _anno = (
        "."
        "/data/MMA-52/Annotations/adatad/mma52_augmented_anno.json"
    )
    _cmap = (
        "."
        "/data/MMA-52/Annotations/adatad/category_idx.txt"
    )
    labels = [ln.strip() for ln in open(_cmap) if ln.strip()]
    counts = {lbl: 0 for lbl in labels}
    for entry in _json.load(open(_anno))["database"].values():
        if entry.get("subset") == "training":
            for ann in entry.get("annotations", []):
                if ann["label"] in counts:
                    counts[ann["label"]] += 1
    total = sum(counts.values())
    n = len(labels)
    raw = [_math.sqrt(total / (n * max(counts[lbl], 1))) for lbl in labels]
    mean_w = sum(raw) / n
    return [round(r / mean_w, 4) for r in raw]


_cls_weights = _compute_class_weights()

window_size = 512
scale_factor = 1
chunk_num = window_size * scale_factor // 16  # 32 chunks

# Override data paths to point at TG videos instead of RGB
data_path_train = "data/tg/train"
data_path_val = "data/tg/val"
data_path_test = "data/tg/test"
...
model = dict(
    ...
    rpn_head=dict(
        ...
        label_smoothing=0.1,
        loss=dict(
            cls_loss=dict(type="FocalLoss", weight=_cls_weights),
...
work_dir = "work_dirs/e2e_mma52_tg_videomae_l_adapter"
