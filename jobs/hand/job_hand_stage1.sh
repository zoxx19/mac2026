#!/bin/bash -l
#SBATCH --job-name=hand_s1
#SBATCH --output=/home/woody/iwso/iwso226h/ma52/logs/hand_stage1.log
#SBATCH --error=/home/woody/iwso/iwso226h/ma52/logs/hand_stage1.err
#SBATCH --gres=gpu:1
#SBATCH --time=12:00:00
#SBATCH --ntasks=1
#SBATCH --cpus-per-task=4

PYTHON=/home/woody/iwso/iwso226h/conda/envs/ma52/bin/python
cd /home/woody/iwso/iwso226h/ma52

$PYTHON src/hand/prepare_twostage.py

$PYTHON src/hand/train_stage1.py \
    --train_csv  data/hand_dataset/hand_stage1_train.csv \
    --val_csv    data/hand_dataset/hand_stage1_val.csv \
    --output_dir outputs/hand_stage1 \
    --model_path models/videomae-ssv2 \
    --batch_size    8 \
    --stage1_epochs 10 \
    --stage2_epochs 20 \
    --lr_stage1     1e-3 \
    --lr_stage2     1e-5 \
    --patience      5
