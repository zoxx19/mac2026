...
#SBATCH --output=logs/infer_val_%j.log
...
SPLIT="val"
SUBSET="validation"
OUT_NAME="${OUT_NAME:-large2_val}"          # -> $PREDICTIONS_DIR/$OUT_NAME.json
...
torchrun --nproc_per_node=1 --master_port=29540 tools/test.py "$CONFIG" \
    --checkpoint "$CKPT" \
    --not_eval \
    --cfg-options \
        work_dir="$TMP" \
