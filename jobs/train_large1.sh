#!/bin/bash
#SBATCH --job-name=train_large1
#SBATCH --account=iwso
#SBATCH --partition=a100
#SBATCH --gres=gpu:a100:2
#SBATCH --time=24:00:00
#SBATCH --cpus-per-task=4
#SBATCH --output=logs/train_large1_%j.log
...
CHECKPOINT_DIR="${CHECKPOINT_DIR:-$PROJECT_DIR/work_dirs/large1}"
PREDICTIONS_DIR="${PREDICTIONS_DIR:-$PROJECT_DIR/predictions}"
CONDA_ENV="${CONDA_ENV:-opentad}"
CONFIG="$OPENTAD_DIR/configs/adatad/mma52/e2e_mma52_videomae_l_adapter.py"   # copy from configs/adatad/large1.py
...
torchrun --nproc_per_node=2 --master_port=29500 tools/train.py "$CONFIG" \
    "${RESUME_ARG[@]}" \
