#!/bin/bash
#SBATCH --job-name=train_asl
#SBATCH --account=iwso
#SBATCH --partition=a100
#SBATCH --gres=gpu:a100:2
#SBATCH --time=24:00:00
#SBATCH --cpus-per-task=4
#SBATCH --output=logs/train_asl_%j.log
...
CHECKPOINT_DIR="${CHECKPOINT_DIR:-$PROJECT_DIR/work_dirs/asl}"
PREDICTIONS_DIR="${PREDICTIONS_DIR:-$PROJECT_DIR/predictions}"
CONDA_ENV="${CONDA_ENV:-opentad}"
CONFIG="$OPENTAD_DIR/configs/adatad/mma52/e2e_mma52_videomae_l_adapter_asl.py"   # copy from configs/adatad/asl.py
...
torchrun --nproc_per_node=2 --master_port=29502 tools/train.py "$CONFIG" \
    "${RESUME_ARG[@]}" \
