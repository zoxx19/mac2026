"""
train_stage1.py
Stage 1: Fine-tunes VideoMAE-base-SSv2 on upper body crops for 5-class
coarse hand recognition (C/E/F/G/no-hand).
Same 2-stage training as head track.

Labels: 0=C, 1=E, 2=F, 3=G, 4=no-hand

Usage:
  python src/hand/train_stage1.py \
      --train_csv data/hand_dataset/hand_stage1_train.csv \
      --val_csv   data/hand_dataset/hand_stage1_val.csv \
      --output_dir outputs/hand_stage1
"""
...
IMG_SIZE    = 224
NUM_CLASSES = 5
ID2LABEL    = {0:'C', 1:'E', 2:'F', 3:'G', 4:'no-hand'}
LABEL2ID    = {v:k for k,v in ID2LABEL.items()}


class HandStage1Dataset(Dataset):
    def __init__(self, csv_path, processor, is_train=True):
        ...
        print(self.df['stage1_label'].value_counts().sort_index().to_string())
...
def main():
    ...
    counts   = train_df['stage1_label'].value_counts().sort_index()
    weights  = torch.tensor(
        [len(train_df)/(NUM_CLASSES*counts.get(i,1))
         for i in range(NUM_CLASSES)], dtype=torch.float).to(device)
    criterion = nn.CrossEntropyLoss(weight=weights)

    train_ds = HandStage1Dataset(args.train_csv, processor, is_train=True)
    val_ds   = HandStage1Dataset(args.val_csv,   processor, is_train=False)
