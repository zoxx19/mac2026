...
def compute_body_bbox(kps, frame_w, frame_h):
    ...
    # Make bbox square but cap expansion to 1.5x the shorter side
    # This prevents over-expansion for tall narrow subjects
    bw = x2 - x1
    bh = y2 - y1
    if bh > bw:
        # person is taller than wide — expand width but cap at 1.5x original width
        target_w = min(bh, int(bw * 1.5))
        diff = target_w - bw
        x1 = max(0, x1 - diff // 2)
        x2 = min(frame_w - 1, x1 + target_w)
    elif bw > bh:
        diff = bw - bh
        y1 = max(0, y1 - diff // 2)
        y2 = min(frame_h - 1, y1 + bw)
    ...
    with mp.solutions.holistic.Holistic(
        ...
        while True:
            ...
            if bbox is not None:
                ...
                crop = cv2.resize(crop, CROP_SIZE) if crop.size > 0 else cv2.resize(frame, CROP_SIZE)
            else:
