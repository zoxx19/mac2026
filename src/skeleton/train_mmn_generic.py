...
Uses paper hyperparameters + STCA augmentation.

Usage:
    python train_mmn_generic.py --track hand --mode full  --num_classes 5
    python train_mmn_generic.py --track hand --mode arm   --num_classes 5
    python train_mmn_generic.py --track leg  --mode full  --num_classes 9
    python train_mmn_generic.py --track leg  --mode leg   --num_classes 9
    python train_mmn_generic.py --track body --mode full  --num_classes 6
    python train_mmn_generic.py --track body --mode torso --num_classes 6

Joint modes:
    full  = all 17 COCO joints
    arm   = shoulders+elbows+wrists      (joints 5-10,  6 joints)
    leg   = hips+knees+ankles            (joints 11-16, 6 joints)
    torso = shoulders+hips               (joints 5,6,11,12, 4 joints)
"""
...
from features import extract_features, ARM_JOINT_IDX
...
# Joint index sets for each mode
JOINT_MODES = {
    "full":  list(range(17)),                  # all 17
    "arm":   [5, 6, 7, 8, 9, 10],             # shoulders+elbows+wrists
    "leg":   [11, 12, 13, 14, 15, 16],        # hips+knees+ankles
    "torso": [5, 6, 11, 12],                  # shoulders+hips
}

# Label column per track
LABEL_COL = {
    "hand": "hand_label",
    "leg":  "leg_label",
    "body": "body_label",
...
class SkeletonDataset(Dataset):
    def __init__(self, csv_path, track, mode="full",
                 num_frames=NUM_FRAMES, training=True):
        self.df         = pd.read_csv(csv_path)
        self.track      = track
        self.mode       = mode
        self.joint_idx  = JOINT_MODES[mode]
    ...
    def __getitem__(self, idx):
        ...
        # Use "full" mode from features.py, then select joints here
        tensor = extract_features(row["kp_path"], mode="full",
                                  num_frames=self.num_frames)  # (2, T, 17, 1)

        # Select joint subset
        tensor = tensor[:, :, self.joint_idx, :]   # (2, T, V, 1)
...
def main():
    ...
    run_name = f"{args.track}_skeleton_{args.mode}"
    out_dir  = OUT_DIR / run_name
    out_dir.mkdir(parents=True, exist_ok=True)

    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print(f"Device     : {device}")
    print(f"Track      : {args.track}  |  Mode: {args.mode}  |  Classes: {args.num_classes}")
    print(f"Run        : {run_name}")

    train_csv  = DATA_DIR / f"{args.track}_train.csv"
    val_csv    = DATA_DIR / f"{args.track}_val.csv"
    label_col  = LABEL_COL[args.track]
    num_joints = len(JOINT_MODES[args.mode])

    train_ds = SkeletonDataset(train_csv, args.track, args.mode, args.num_frames, training=True)
    val_ds   = SkeletonDataset(val_csv,   args.track, args.mode, args.num_frames, training=False)
    ...
    print(f"Train      : {len(train_ds)} | Val: {len(val_ds)} | Joints: {num_joints}")
    ...
    class_weights = compute_class_weights(train_csv, label_col, args.num_classes, device)
