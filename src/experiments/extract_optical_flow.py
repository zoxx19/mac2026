...
Extracts optical flow from leg crop videos using OpenCV Farneback method.
Saves flow as grayscale magnitude+direction images stacked into MP4.

For each video: computes frame-to-frame optical flow, 
encodes magnitude and angle as 2-channel flow video (224x224).
...
def extract_flow_video(video_path, out_path, size=224):
    """
    Extract optical flow from video and save as grayscale MP4.
    Each frame = magnitude of flow encoded as grayscale.
    """
    ...
    while True:
        ...
        # Encode angle as hue (0-179 for OpenCV HSV)
        hsv = np.zeros((size, size, 3), dtype=np.uint8)
        hsv[..., 0] = ang * 180 / np.pi / 2   # hue = direction
        hsv[..., 1] = 255                       # saturation = full
        hsv[..., 2] = mag.astype(np.uint8)      # value = magnitude
...
def main():
    ...
    cfg = {
        'train': {
            'csv':     'data/skeleton_dataset/leg_train_videomae.csv',
            'out_dir': 'data/leg_flow/train',
            'csv_out': 'data/skeleton_dataset/leg_train_flow.csv',
        },
        'val': {
            'csv':     'data/skeleton_dataset/leg_val_videomae.csv',
            'out_dir': 'data/leg_flow/val',
            'csv_out': 'data/skeleton_dataset/leg_val_flow.csv',
        },
    }[args.split]
