"""
train_body_oversample.py
VideoMAE fine-tuning on oversampled body crops with heavier augmentation
for rare classes (A3, A4).
...
class BodyOversampleDataset(Dataset):
    def __init__(self, csv_path, processor, is_train=True):
        ...
        print(self.df['body_label'].value_counts().sort_index().to_string())
    ...
    def _augment_heavy(self, frames, label):
        ...
        if np.random.random() > 0.5 or label in [2, 3]:
    ...
    def __getitem__(self, idx):
        ...
        label = int(row['body_label'])

        # Speed perturbation for rare classes
        speed = 1.0
        if self.is_train and label in [2, 3]:
...
def main():
    ...
    counts   = train_df['body_label'].value_counts().sort_index()
    ...
    train_ds = BodyOversampleDataset(args.train_csv, processor, is_train=True)
    val_ds   = BodyOversampleDataset(args.val_csv,   processor, is_train=False)
