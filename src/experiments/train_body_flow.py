"""
train_body_flow.py
VideoMAE-SSv2 fine-tuning on optical flow body videos.
Same approach as train_leg_flow.py but for body track.
...
def main():
    ...
    print(f"Device: {device} | Optical Flow Body | Classes: {NUM_CLASSES}")
    ...
    train_ds     = BodyFlowDataset(args.train_csv, processor, is_train=True)
    val_ds       = BodyFlowDataset(args.val_csv,   processor, is_train=False)
    ...
    print(f"\nDone. Optical Flow Body | Best F1_mean: {best_f1:.4f}")
