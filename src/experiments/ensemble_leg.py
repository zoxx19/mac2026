...
from train_leg_flow import LegFlowDataset
...
def get_flow_probs(flow_model_path, val_csv, model_path, device):
    ...
    val_ds     = LegFlowDataset(val_csv, processor, is_train=False)
...
def get_mmn_probs(mmn_model_path, val_csv, device):
    ...
    model = MMN_(num_classes=NUM_CLASSES, num_points=6)
    ...
    with torch.no_grad():
        for _, row in tqdm(df.iterrows(), total=len(df), desc='MMN inference'):
            ...
            labels.append(int(row['leg_label']))
...
def main():
    ...
    p.add_argument('--flow_model', default='outputs/leg_flow_ssv2/best_model.pt')
    p.add_argument('--mmn_model',  default='outputs/leg_leg_joint_aug/best_model.pt')
    p.add_argument('--flow_csv',   default='data/skeleton_dataset/leg_val_flow.csv')
    p.add_argument('--mmn_csv',    default='data/skeleton_dataset/leg_val.csv')
